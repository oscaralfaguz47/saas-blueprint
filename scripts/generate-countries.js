/**
 * One-time script: writes src/lib/countries.ts from ISO 3166-1 alpha-2 data.
 * Run: node scripts/generate-countries.js
 * Uses embedded data (no network required).
 */
const fs = require("fs");
const path = require("path");

// ISO 3166-1 alpha-2 (lukes/ISO-3166-Countries-with-Regional-Codes), name + alpha-2 only
const RAW = [
  ...
  ["NL", "Netherlands, Kingdom of the"], ["NC", "New Caledonia"], ["NZ", "New Zealand"], ["NI", "Nicaragua"],
  ...
  ["UA", "Ukraine"], ["AE", "United Arab Emirates"], ["GB", "United Kingdom of Great Britain and Northern Ireland"], ["US", "United States of America"],
...
function main() {
  const list = RAW.map(([code, name]) => ({ value: code, label: name }))
    .sort((a, b) => a.label.localeCompare(b.label));

  const out = `/** ISO 3166-1 alpha-2 country list (English names), sorted A–Z. For billing address selection. */
export type CountryOption = { value: string; label: string };

const COUNTRIES: CountryOption[] = ${JSON.stringify(list, null, 2)};
