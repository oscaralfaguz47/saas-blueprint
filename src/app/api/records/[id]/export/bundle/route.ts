...
/**
 * POST /api/records/[id]/export/bundle
 * I2 — Enqueue ZIP audit bundle (plan zip feature + ZIP_EXPORTS meter).
 */
export const POST = withErrorHandler(async (
  ...
  if (!plan.features.zip.enabled) {
    return ApiErrors.UPGRADE_REQUIRED("Audit bundle export requires a Pro plan.");
  }

  await checkMeterLimit({ tenantId, meter: "ZIP_EXPORTS", delta: 1 });

  const rl = await checkRateLimit(`export:zip:${session.user.id}:${recordId}`, 2, 60 * 1000);
  if (!rl.allowed) {
    return ApiErrors.RATE_LIMITED("Bundle export already in progress.", {
  ...
  const job = await enqueueBackgroundJobReturning({
    tenantId,
    jobType: JOB_TYPES.EXPORT_ZIP_BUNDLE,
    idempotencyKey: `export:zip:${tenantId}:${recordId}:${randomUUID()}`,
    payload: {
      recordId,
